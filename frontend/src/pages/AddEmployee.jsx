...
export default function AddEmployee() {
  ...
  const createMutation = useMutation(employeeService.createEmployee, {
    onSuccess: async (responseData) => {
      ...
      const employeeId =
        (created && (created.id || created.employee_id || created.employee_id)) ||
        resp.id ||
        resp.employee_id ||
        resp.data?.id ||
      ...
      if (!employeeId) {
        toast.warning("Employee created but could not detect ID for auto assignment. Please assign the salary structure manually.");
        navigate("/employer/employees");
        return;
      }
      ...
      if (selectedStructure) {
        const effective_from = formData.hire_date || new Date().toISOString().split("T")[0];
  ...
  const handleStructureChange = async (e) => {
    ...
    if (found) {
      setFormData(prev => ({ ...prev, basic_salary: found.basic_salary || "" }));
      return;
    }
    ...
    try {
      ...
      setFormData(prev => ({ ...prev, basic_salary: structure.basic_salary || "" }));
    } catch (err) {
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <TextField
                ...
                helperText={selectedStructure ? "Salary locked to structure" : "Select structure to auto-fill salary"}
              >
                <MenuItem value="">
                  <em>None</em>
                </MenuItem>
